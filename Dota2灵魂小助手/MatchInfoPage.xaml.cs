...
namespace Dota2灵魂小助手
{
    ...
    public sealed partial class MatchInfoPage : Page
    {
        ...
        private async void ShowPlayers(string match_id)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                radiantPlayersList.Add(new PlayersInfoViewModel(players[i]) { PlayerName = "匿名玩家", PlayerPhoto = "ms-appx:///Assets/Pictures/null.png" });
            ...
            for (int i = 5; i < 10; i++)
            {
                direPlayersList.Add(new PlayersInfoViewModel(players[i]) { PlayerName = "匿名玩家", PlayerPhoto = "ms-appx:///Assets/Pictures/null.png" });
        ...
        private async void ShowPlayersPhoto(List<PlayersInfoViewModel> radiant, List<PlayersInfoViewModel> dire)
        {
            foreach (PlayersInfoViewModel item in radiant)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                item.PlayerName = personanameMatch.Groups[1].Value == "" ? "匿名玩家" : personanameMatch.Groups[1].Value;
                    ...
                    {
                        ShowDialog("查询数据太频繁啦，让服务器先休息一下吧~");
                        return;
                    }
                }
            }
